...
FCustomPlacementMode::FCustomPlacementMode()
	: AssetsToPlace()
...
void FCustomPlacementMode::Initialize()
{
...
bool FCustomPlacementMode::UsesToolkits() const
{
	return true;
}

void FCustomPlacementMode::Enter()
{
	...
	if (!Toolkit.IsValid())
	{
		Toolkit = MakeShareable(new FCustomPlacementModeToolkit);
		Toolkit->Init(Owner->GetToolkitHost());
	}
}

void FCustomPlacementMode::Exit()
{
	if (Toolkit.IsValid())
	{
...
void FCustomPlacementMode::Tick(FEditorViewportClient* ViewportClient, float DeltaTime)
{
...
bool FCustomPlacementMode::MouseEnter(FEditorViewportClient* ViewportClient, FViewport* Viewport, int32 x, int32 y)
{
	if (IsCurrentlyPlacing())
	{
		ViewportClient->SetRequiredCursorOverride(true, EMouseCursor::GrabHandClosed);
...
bool FCustomPlacementMode::MouseLeave(FEditorViewportClient* ViewportClient, FViewport* Viewport)
{
...
bool FCustomPlacementMode::AllowPreviewActors(FEditorViewportClient* ViewportClient) const
{
	return (IsCurrentlyPlacing() && (ViewportClient->IsTracking() && AllowPreviewActorsWhileTracking)) || !ViewportClient->IsTracking();
}

bool FCustomPlacementMode::MouseMove(FEditorViewportClient* ViewportClient, FViewport* Viewport, int32 x, int32 y)
{
	UpdatePreviewActors(ViewportClient, Viewport, x, y);
	return FEdMode::MouseMove(ViewportClient, Viewport, x, y);
}

void FCustomPlacementMode::UpdatePreviewActors(FEditorViewportClient* ViewportClient, FViewport* Viewport, int32 x, int32 y)
{
...
bool FCustomPlacementMode::InputKey(FEditorViewportClient* InViewportClient, FViewport* InViewport, FKey InKey, EInputEvent InEvent)
{
	bool Handled = false;
	...
	if (IsCurrentlyPlacing())
	{
		...
		else if (!bIsCtrlDown)
		{
			...
			if (PlacedActors.Num() > 0)
			{
				...
				ClearAssetsToPlace();
				ICustomPlacementModeModule::Get().BroadcastStoppedPlacing(true);
			}
		}
	}
...
bool FCustomPlacementMode::StartTracking(FEditorViewportClient* InViewportClient, FViewport* InViewport)
{
...
bool FCustomPlacementMode::EndTracking(FEditorViewportClient* InViewportClient, FViewport* InViewport)
{
	if (IsCurrentlyPlacing())
	{
		InViewportClient->SetRequiredCursorOverride(true, EMouseCursor::GrabHandClosed);
...
bool FCustomPlacementMode::HandleClick(FEditorViewportClient* InViewportClient, HHitProxy *HitProxy, const FViewportClick &Click)
{
	bool Handled = false;
...
bool FCustomPlacementMode::InputDelta(FEditorViewportClient* InViewportClient, FViewport* InViewport, FVector& InDrag, FRotator& InRot, FVector& InScale)
{
	if (IsCurrentlyPlacing())
	{
...
bool FCustomPlacementMode::ShouldDrawWidget() const
{
	return IsCurrentlyPlacing() ? false : FEdMode::ShouldDrawWidget();
}

bool FCustomPlacementMode::UsesPropertyWidgets() const
{
	return IsCurrentlyPlacing() ? false : FEdMode::ShouldDrawWidget();
}

bool FCustomPlacementMode::IsCompatibleWith(FEditorModeID OtherModeID) const
{
	return
...
void FCustomPlacementMode::StartPlacing(const TArray< UObject* >& Assets, UActorFactory* Factory)
{
	...
	ICustomPlacementModeModule::Get().BroadcastStartedPlacing(Assets);
}

void FCustomPlacementMode::StopPlacing()
{
	if (IsCurrentlyPlacing())
	{
		ClearAssetsToPlace();
		ICustomPlacementModeModule::Get().BroadcastStoppedPlacing(false);
		PlacementsChanged = true;
	}
}

void FCustomPlacementMode::SetPlacingFactory(UActorFactory* Factory)
{
...
UActorFactory* FCustomPlacementMode::FindLastUsedFactoryForAssetType(UObject* Asset) const
{
	if (Asset == NULL)
	{
		return NULL;
...
void FCustomPlacementMode::ClearAssetsToPlace()
{
	AssetsToPlace.Empty();
	PlacedActors.Empty();
	PlacementsChanged = true;
...
void FCustomPlacementMode::SelectPlacedActors()
{
...
const TArray< TWeakObjectPtr<UObject> >& FCustomPlacementMode::GetCurrentlyPlacingObjects() const
{
