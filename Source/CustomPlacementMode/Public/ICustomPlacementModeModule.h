...
#include "ICustomPlacementMode.h"
 ...
 */
class FCustomPlacementModeID
{
public:
	FCustomPlacementModeID(const FCustomPlacementModeID&) = default;
	FCustomPlacementModeID& operator=(const FCustomPlacementModeID&) = default;

private:
	friend class FCustomPlacementModeModule;

	FCustomPlacementModeID() {}
 ...
 */
struct FCustomPlacementCategoryInfo
{
	FCustomPlacementCategoryInfo(FText InDisplayName, FName InHandle, FString InTag, int32 InSortOrder = 0, bool bInSortable = true)
 ...
 */
struct FCustomPlaceableItem
{
	/** Default constructor */
	FCustomPlaceableItem()
		: Factory(nullptr)
	{}


	/** Constructor that takes a specific factory and asset */
	FCustomPlaceableItem(UActorFactory* InFactory, const FAssetData& InAssetData, TOptional<int32> InSortOrder = TOptional<int32>())
		: Factory(InFactory)
	...
	FCustomPlaceableItem(UClass& InAssetClass, TOptional<int32> InSortOrder = TOptional<int32>())
...
struct FCustomBuiltInPlacementCategories
{
	...
	static FName Custom() { static FName Name("Custom");		return Name; }
};
...
public:
	...
	FCategoryItem(FName inHandleName, const FName inDisplayName, const FString inTagName)
	{
...
class ICustomPlacementModeModule : public IModuleInterface
{
public:
	 ...
	 */
	static inline ICustomPlacementModeModule& Get()
	{
		return FModuleManager::LoadModuleChecked< ICustomPlacementModeModule >("CustomPlacementMode");
	}
	...
	{
		return FModuleManager::Get().IsModuleLoaded("CustomPlacementMode");
	}

	/**
	 * Add the specified assets to the recently placed items list
	 */
	virtual void AddToRecentlyPlaced(const TArray< UObject* >& Assets, UActorFactory* FactoryUsed = nullptr) = 0;

	/**
	 * Add the specified asset to the recently placed items list
	 */
	virtual void AddToRecentlyPlaced(UObject* Asset, UActorFactory* FactoryUsed = nullptr) = 0;

	/**
	 * Get a copy of the recently placed items
	 */
	virtual const TArray< FActorCustomPlacementInfo >& GetRecentlyPlaced() const = 0;

	/**
	 * @return the event that is broadcast whenever the list of recently placed assets changes
	 */
	DECLARE_EVENT_OneParam(ICustomPlacementMode, FOnRecentlyPlacedChanged, const TArray< FActorCustomPlacementInfo >& /*NewRecentlyPlaced*/);
	 ...
	 */
	DECLARE_EVENT(ICustomPlacementMode, FOnAllPlaceableAssetsChanged);
	virtual FOnAllPlaceableAssetsChanged& OnAllPlaceableAssetsChanged() = 0;

	/**
 * @return the event that is broadcast whenever the list of all custom placeable assets changes
 */
	DECLARE_EVENT(ICustomPlacementMode, FOnAllCustomPlaceableAssetsChanged);
	virtual FOnAllCustomPlaceableAssetsChanged& OnAllCustomPlaceableAssetsChanged() = 0;

	/**
	 * @return the event that is broadcast whenever a placement mode enters a placing session
	 */
	DECLARE_EVENT_OneParam(ICustomPlacementMode, FOnStartedPlacingEvent, const TArray< UObject* >& /*Assets*/);
	 ...
	 */
	DECLARE_EVENT_OneParam(ICustomPlacementMode, FOnStoppedPlacingEvent, bool /*bWasSuccessfullyPlaced*/);
...
public:
	 ...
	 */
	virtual bool RegisterPlacementCategory(const FCustomPlacementCategoryInfo& Info) = 0;
	 ...
	 */
	virtual const FCustomPlacementCategoryInfo* GetRegisteredPlacementCategory(FName UniqueHandle) const = 0;
	 ...
	 */
	virtual void GetSortedCategories(TArray<FCustomPlacementCategoryInfo>& OutCategories) const = 0;
	 ...
	 */
	virtual TOptional<FCustomPlacementModeID> RegisterPlaceableItem(FName CategoryName, const TSharedRef<FCustomPlaceableItem>& Item) = 0;
	 ...
	 */
	virtual void UnregisterPlaceableItem(FCustomPlacementModeID ID) = 0;
	 ...
	 */
	virtual void GetItemsForCategory(FName Category, TArray<TSharedPtr<FCustomPlaceableItem>>& OutItems) const = 0;
	 ...
	 */
	virtual void GetFilteredItemsForCategory(FName Category, TArray<TSharedPtr<FCustomPlaceableItem>>& OutItems, TFunctionRef<bool(const TSharedPtr<FCustomPlaceableItem>&)> Filter) const = 0;
	...
	virtual TArray<FCategoryItem> GetCustomCategories() = 0;
};
