 ...
 * of each tab.
 */
class SCustomPlacementAssetEntry : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SCustomPlacementAssetEntry){}
	...
	SLATE_END_ARGS()
		...
		void Construct(const FArguments& InArgs, const TSharedPtr<const FCustomPlaceableItem>& InItem);
	...
	TSharedPtr<const FCustomPlaceableItem> Item;
...
class SCustomPlacementModeTools : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SCustomPlacementModeTools) {}
	SLATE_END_ARGS();

	void Construct(const FArguments& InArgs);

	virtual ~SCustomPlacementModeTools();
...
private:
	...
	TSharedRef< SWidget > CreatePlacementGroupTab(const FCustomPlacementCategoryInfo& Info);

	/** Creates a tab widget to show on the left that when clicked sets the currently active tab */
	TSharedRef< SWidget > CreateCustomPlacementGroupTab(const FCustomPlacementCategoryInfo& Info);

	TSharedRef< SWidget > GenerateColorsPlacementGroupTab(const FSettingDefaultPlaceableCategoryItem Item, const FCustomPlacementCategoryInfo& Info);

	/** Generates a widget for the specified item */
	TSharedRef<ITableRow> OnGenerateWidgetForItem(TSharedPtr<FCustomPlaceableItem> InItem, const TSharedRef<STableViewBase>& OwnerTable);
	...
	void UpdateRecentlyPlacedAssets(const TArray< FActorCustomPlacementInfo >& RecentlyPlaced);

	/** Called when the list of placeable assets changes. */
	void UpdatePlaceableAssets();

	/** Called when the list of custom placeable assets changes. */
	void UpdateCustomPlaceableAssets();
...
private:
	...
	bool IsCutomCategory(FCustomPlacementCategoryInfo Category);
...
private:
	...
	bool bCustomPlaceablesRefreshRequested;

	/** Flag to indicate that we need to update the filtered items array */
	bool bNeedsUpdate;

	// The text filter used to filter the classes
	typedef TTextFilter<const FCustomPlaceableItem&> FPlacementAssetEntryTextFilter;
	...
	TArray<TSharedPtr<FCustomPlaceableItem>> FilteredItems;

	/** The name of the currently active tab (where no search is active) */
	FName ActiveTabName;

	/** List view that shows placeable items */
	TSharedPtr<SListView<TSharedPtr<FCustomPlaceableItem>>> ListView;
};

DECLARE_LOG_CATEGORY_EXTERN(CustomPlacementModeToolsLog,Log,All)